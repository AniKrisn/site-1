...
export class GeminiLiveSession {
	...
	private dispatchNarration(text: string, onPlayStart?: () => void): void {
		...
		// Keep this prompt minimal so the model treats the text as a natural
		// continuation of its previous turn, not a "perform this script"
		// instruction (which shifts its tone into announcer-mode).
		this.sendJson({
			clientContent: {
				turns: [
					{
						...
						parts: [
							{
								text: `(continuing your same reply, same voice and pace, no preamble)\n\n${text}`,
							},
						],
					},
				],
	...
	/**
	 * Pre-create and resume the output AudioContext on a user gesture so
	 * the browser's autoplay policy doesn't silently mute Live's audio.
	 * Call this from a handler that's running inside a user-initiated
	 * event (e.g. keydown on M).
	 */
	primeAudio(): void {
		if (!this.outputCtx) {
	...
	private scheduleAudio(samples: Float32Array) {
		...
		// Best-effort resume — if we missed the user gesture, this no-ops
		// silently. Always called so a previously-suspended context can
		// catch up if a fresh gesture happened recently.
		if (this.outputCtx.state === 'suspended') {
			this.outputCtx.resume().catch(() => {})
		}
